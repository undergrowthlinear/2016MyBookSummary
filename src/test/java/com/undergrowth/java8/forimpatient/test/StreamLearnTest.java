...
public class StreamLearnTest {
    ...
    public void before() throws URISyntaxException, IOException {
        String contents = new String(Files.readAllBytes(Paths.get(StreamLearnTest.class.getClassLoader().getResource("war-and-peace.txt").toURI())), StandardCharsets.UTF_8);
    ...
    public void limitStreamTest(){
        ...
        Double[] strings=Stream.iterate(1.0,p->p*2).peek(e-> System.out.println("Fetching-->"+e)).limit(20).toArray(Double[]::new);
        for (Double str:
        strings) {
    ...
    public  void collectorsTest(){
        List<Person> persons=Arrays.asList(new Person("under",18,100,new Department("1号")),new Person("growth",25,500,new Department("1号")),new Person("qqwwee",30,1000,new Department("2号")));
        //转换list
    ...
    public void collectors2Test(){
        List<Person> persons=Arrays.asList(new Person("under",18,100,new Department("1号"),50),new Person("growth",25,500,new Department("1号"),80),new Person("qqwwee",30,1000,new Department("2号"),20));
        Map<Department, List<Person>> byDept = persons.stream().collect(Collectors.groupingBy(Person::getDepartment));
        System.out.println("计算部门人员");
        for (Map.Entry<Department, List<Person>> departPersonEntry:
        byDept.entrySet()) {
            System.out.println(departPersonEntry.getKey().getDeptName()+"---->"+departPersonEntry.getValue().size());
        }
        //计算部门薪水
        System.out.println("计算部门薪水");
        Map<Department, Integer> totalByDept= persons.stream().collect(Collectors.groupingBy(Person::getDepartment,  Collectors.summingInt(Person::getSalary)));
        ...
        Map<Boolean, List<Person>> passingFailing = persons.stream().collect(Collectors.partitioningBy(s -> s.getGrade() >= 60));
