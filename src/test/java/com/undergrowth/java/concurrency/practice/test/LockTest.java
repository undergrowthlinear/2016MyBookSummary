...
/**
 * Description: TODO(这里用一句话描述这个类的作用) Q1:内置锁与显示锁的区别 A1:synchronized----内置锁----当资源被占用时,调用者进入休眠状态 Lock----显示锁----支持无条件/可中断/可定时/可轮询 Q2:乐观锁与悲观锁的区别
 * Q2:悲观锁在每次访问数据时,都进行加锁操作,适用于多写的场景 乐观锁----每次访问数据不进行加锁,当更新操作时,会去回补哪些操作过数据,适用于多读的场景 Q3:自旋锁 A3:程序通过循环等待而不是休眠
 * see----ReentrantLockTest/ReentrantReadWriteLockTest 内置锁与显示锁相比---- 显示锁提供了一些扩展的功能,例如可定时/可中断/可轮询,性能方面差别不大,与内置锁具有相同的互斥性/内存可见性/可重入
