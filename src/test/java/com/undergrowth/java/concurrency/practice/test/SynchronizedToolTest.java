...
/**
 * Description: TODO(同步工具类) synchronized----使用对象的内置锁(互斥锁)与内置条件队列进行同步/wait nofity nitofyAll为内部条件队列的api/可重入(获取锁的操作粒度是线程)
 * CountDownLatch--为同步工具类之Latch(闭锁)--多个线程等待一个任务/一个线程等待多个任务/一次性状态 FutureTask--表示一种抽象的可生成结果的运算 Semaphore--控制同时访问特定资源的数目/不可重入
